...
class _FMCGSDStoresState extends State<FMCGSDStores>
  ...
  Future<void> _initDatabase() async {
    ...
    String path = join(directory.path, 'fmcg_stores.db');
    ...
    _database = await openDatabase(
      ...
      onCreate: (db, version) async {
        await db.execute(
          'CREATE TABLE stores(id INTEGER PRIMARY KEY, store_name TEXT, store_id TEXT, date TEXT, status TEXT)',
        );
      },
    );
    ...
    await _database.transaction((txn) async {
      await txn.insert('stores', {
        'store_name': 'Zannat General Store',
        'store_id': '9665321',
        'date': '28 Jan, 2024',
        'status': 'Completed (RA)',
      });
      await txn.insert('stores', {
        'store_name': 'Sadat General Store',
        'store_id': '8665321',
        'date': '28 Jan, 2024',
        'status': 'Pending (RA)',
      });
      await txn.insert('stores', {
        'store_name': 'Rahat General Store',
        'store_id': '7665321',
        'date': '28 Jan, 2024',
        'status': 'Pending (PC)',
      });
  ...
  Future<void> _loadData() async {
    final data = await _database.query('stores');
    if (mounted) {
      setState(() {
        _data = data;
      });
    ...
    return ListTile(
      ...
      onTap: () {
        ...
        if (option == 'Temporary Closed (TC)' ||
          ...
          Get.to(() => FmcgSdStoreDetails(item: item)); // Navigate
        }
      },
    );
  }
