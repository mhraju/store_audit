...
class _FmcgSdSkuListState extends State<FmcgSdSkuList> {
  ...
  void _showBottomSheet(Map<String, dynamic> skuItem) {
    String previousMrp = '';
    String previousClosingStock = '';
    dbManager
        .getPrevMrpAndStockFmcgSd(
            widget.dbPath, widget.auditorId, widget.storeCode, skuItem['code'])
        .then((prevData) {
      print("Previous Month MRP: ${prevData['prevMrp']}");
      print("Previous Month Closing Stock: ${prevData['prevClosingStock']}");

      setState(() {
        previousMrp = prevData['prevMrp'] ?? '0';
        previousClosingStock = prevData['prevClosingStock'] ?? '0';
      });
    });
...
// Initialize controllers with improved logic
    ...
    void updateSaleValue() {
      int openingStock = int.tryParse(previousClosingStock.toString()) ?? 0;
      int purchase =
      ...
      setState(() {
        ...
        editedValues[itemName] = {
          ...
          'closestock': closingStock.toString(),
          'sale': saleValue.toString(),
          //'mrp': mrpController.text,
        };
      });

      // ✅ If sale is negative, reset Closing Stock & show alert
      if (calculatedSale < 0) {
        setState(() {
    ...
    void updateMrpValue() {
      ...
      double? lastMrpFromDb = double.tryParse(previousMrp);

      print(lastMrpFromDb);
      if (newMrp == null || newMrp <= 0) {
        // ✅ Prevent empty or zero MRP values
        mrpController.text = lastMrpFromDb.toString();
        return;
      }

      // ✅ Calculate allowed range (±20%)
      double minAllowed = lastMrpFromDb! * 0.8;
      double maxAllowed = lastMrpFromDb * 1.2;

      if (newMrp < minAllowed || newMrp > maxAllowed) {
        // ✅ Show alert if deviation is more than ±20%
        showDialog(
          context: context,
          builder: (BuildContext context) {
            return AlertDialog(
              ...
              content: Text(
                "The new MRP (\$newMrp) is outside the allowed 20% deviation range.\n\n"
        ...
        // ✅ Reset MRP to last valid value
        mrpController.text = lastMrpFromDb.toString();
        return;
      }

      // ✅ If MRP is within range, update it
      setState(() {
        editedValues[itemName]?['mrp'] = newMrp.toString();
      });
    }
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildNonEditableField(
                        'Opening Stock (OS)', previousClosingStock.toString()),
                    ...
                    _buildEditableField(
                        ...
                        controller: mrpController, onChanged: updateMrpValue),
                    ...
                    Center(
                      child: ElevatedButton(
                        onPressed: () async {
                          ...
                          // ✅ If closing stock is 0, set MRP to 0
                          if (closingStock == 0) {
                            mrpController.text = '0';
                          }

                          // ✅ Check if closing stock > 0 and MRP is empty or 0
                          if (closingStock > 0 &&
                              (mrpValue == null || mrpValue == 0)) {
                            showDialog(
                              context: context,
                              builder: (BuildContext context) {
                                return AlertDialog(
                          ...
                          setState(() {
                            ...
                            editedValues[itemName] = {
                              ...
                              'wholesale': double.tryParse(
                                          wholesaleController.text.trim())
                                      ?.round() ??
                                  0,
                              'mrp': mrpController.text,
                              'avgSaleLastMonth': double.tryParse(
                                          avgSaleLastMonthController.text
                                              .trim())
                                      ?.round() ??
                                  0,
                              'avgSaleLastToLastMonth': double.tryParse(
